...
from .strategies import bfs_path, distance_to_end_map, shortest_visible_value, visible_cells, clamp
...
class ProbeAI:
    ...
    def __init__(
        ...
        self.path = bfs_path(maze, maze.start, maze.end)
        self.path_index = 0
        self.detour_limit = 6
    ...
    def step(self) -> ProbeState:
        ...
        next_on_path = self._next_path_step()
        if next_on_path is not None:
            self.position = next_on_path
            self.steps += 1
    ...
    def _next_path_step(self) -> tuple[int, int] | None:
        if not self.path:
            return None

        if self.path_index < len(self.path) and self.path[self.path_index] == self.position:
            if self.path_index + 1 < len(self.path):
                self.path_index += 1
                return self.path[self.path_index]
            return None

        self._rebuild_path_with_coins()
        if not self.path or len(self.path) <= 1:
            return None
        self.path_index = 1
        return self.path[self.path_index]

    def _rebuild_path_with_coins(self) -> None:
        base_path = bfs_path(self.maze, self.position, self.maze.end)
        if not base_path:
            self.path = []
            self.path_index = 0
            return
        ...
        coins = self._available_coins()
        for coin in coins:
            to_coin = bfs_path(self.maze, self.position, coin)
            if not to_coin:
                continue
            coin_to_end = bfs_path(self.maze, coin, self.maze.end)
            if not coin_to_end:
                continue
            total_len = len(to_coin) + len(coin_to_end) - 1
            if total_len <= base_len + self.detour_limit and total_len < best_len:
                best_len = total_len
                best_path = to_coin + coin_to_end[1:]

        self.path = best_path
        self.path_index = 0

    def _available_coins(self) -> list[tuple[int, int]]:
        coins: list[tuple[int, int]] = []
        for row in range(self.maze.rows):
            for col in range(self.maze.cols):
                cell = (row, col)
                if cell in self.collected_coins:
                    continue
                if self.maze.grid[row][col].content == SYMBOLS["coin"]:
                    coins.append(cell)
        return coins
