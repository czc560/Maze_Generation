...
def distance_map(maze: Maze, start: tuple[int, int]) -> dict[tuple[int, int], int]:
    q: deque[tuple[int, int]] = deque([start])
...
def optimal_path_max_coins(
    maze: Maze,
    detour_limit: int = 12,
    coin_mask_limit: int = 16,
) -> list[tuple[int, int]]:
    if maze.start is None or maze.end is None:
        return []
    ...
    def coin_mask_for(cell: tuple[int, int]) -> int:
    ...
    queue: deque[tuple[tuple[int, int], int]] = deque([start_state])
    steps_map: dict[tuple[tuple[int, int], int], int] = {start_state: 0}
    parent: dict[tuple[tuple[int, int], int], tuple[tuple[int, int], int]] = {}

    best_state: tuple[tuple[int, int], int] | None = None
    best_coins = -1
    best_steps = 0
    ...
    while queue:
        ...
        if pos == end:
            ...
            if coins_collected > best_coins or (
                coins_collected == best_coins and steps < best_steps
            ):
                best_state = (pos, mask)
